...
for parallel_file in "$parallel_dir"/gol_output_parallel_*.txt; do
    filename=$(basename "$parallel_file")

    timestep="${filename#gol_output_parallel_}"
    timestep="${timestep%.txt}"

    serial_file="$serial_dir/gol_output_serial_${timestep}.txt"
