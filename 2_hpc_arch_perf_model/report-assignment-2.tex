...
The algorithmic intensity is assumed to be a constant, given by the compute architecture and clock frequency.
We assume that no SIMD instructions are being used and that the pipeline data width equals one Byte and thus is able to archeive 1 Floating point operation per instruction cycle.
With the clock frequency of $2.25 GHz$, we calculate based on our assumptions an algorithmic intensity (AI) of $ 2.25 GFLOP/Byte $.
...
The measured FLOP/s (Operational Intensity) of our sample workload are ... to the peak FLOP/s of the system, determined via the ... \todo{question 5} .

The Operational Intensity [FLOPs/Byte] of the sample workload is related to our array size $N$.
It scales with larger array sizes. \todo{question 6}.
