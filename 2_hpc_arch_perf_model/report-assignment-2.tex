...
For the addition, we get a bandwidth of $21497.8 MB/s$.
...
Using the AI and the Bandwidth, we can determine the Performance defined as $Performance = BW * AI$.

Running the given C program gives us the following results for different N sizes:
{1000: 0.000007,10000: 0.000066,100000: 0.000655,1000000: 0.004609,10000000: 0.048994}

We can thus calculate the FLOPs as $FLOPs = N/t_execute$:

We can calculate the Operational Intensity (OI) as $OI = N/24$:

In the plots below we can see the Operational Intensity vs. Performance with regard to the roofline model. 
Simulations for $10e8 Bytes$ and larger array sizes fail to run on one CPU due to running out of memory (Slurm returning OOM error). We adress this by allocating additional memory of 200GB for the simulations for $10e8 B$ and above.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{img/bonus_lim1024}
  \caption{arithmetic Intensity (x-axis)[FLOPs/Byte], Performance (y-axis)[GFLOPs]}
  \label{fig:roofline}
\end{figure}

The dots in the figure indicate our measurements for the given code simulating a compuational workload
on a node. We can observe, that we are closer to the Bandwidth limit, indicating that our computation is 
memory-bound. \todo{recheck}

The measured FLOP/s (Operational Intensity) of our sample workload is close to the peak FLOP/s of the system as shown in the plot. Hence the computation is compute bound. 

The Operational Intensity [FLOPs/Byte] of the sample workload is related to our array size $N$.
It scales with larger array sizes. Since the sample code does perform a vector addition, where each element is used once, the data is not reused and thus result in low oper

To move the workload closer to the compute bound we could apply techniques as blocking to more efficiently use
the cache size and minimize memory fetching to a necessary minimum.
