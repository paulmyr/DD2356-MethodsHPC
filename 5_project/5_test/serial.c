...
int main() {
    ...
    // Expected value = x0 + c * t
    // c_FDTD = propagation speed based on CFL: c_FDTD = DX/DT
    double c_FDTD = DX/DT;
    double center = NX * DX / 2.0;
    double x_expected = center + c_FDTD * NSTEPS * DT;
    printf("Expected value: %f, relative error: %f \n", x_expected, (max - x_expected)/x_expected);
