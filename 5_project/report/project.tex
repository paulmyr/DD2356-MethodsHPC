...
However, as the grid sizes increase, we start seeing the noticable benefit of using async over sync communication for halo-exchanges. As the grid sizes increase, so does the "computation" part of the overlap -- meaning there is a non-trivial amount of wait-time which allows for the \verb|MPI_Waitall| to finish quickly and bring forth the anticipated advantage.
