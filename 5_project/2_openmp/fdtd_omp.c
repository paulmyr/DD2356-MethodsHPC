...
void print_to_file(double *E, double *H, int NX, int step) {
  ...
  sprintf(filename_E, "outputs/parallel_omp/fdtd_parallel_E_%d.txt", step);
  sprintf(filename_H, "outputs/parallel_omp/fdtd_parallel_H_%d.txt", step);
...
//   print_to_file(E, H, NX, NSTEPS + 1);
  ...
  printf("[PARALLEL] Time Taken (%d grid, %d steps, %d threads): %f\n", NX,
         NSTEPS, NTHREADS, end_time - start_time);
